...
## 11. Profondeur historique réellement disponible

Mesurée après `make backfill` le 2026-09-05. Rien n'est extrapolé : quand une
source n'a pas de données pour une période, cette période n'existe pas en base.

### Bougies (Binance)

| Actif | 1d | 4h | 1h | 1w |
|---|---|---|---|---|
| **BTC** | 3306 barres, **2017-08-17 →** (9,0 ans) | 6000, 2023-12 → | 10000, 2025-07 → | 473, 2017-08 → |
| **ETH** | 3306 barres, **2017-08-17 →** (9,0 ans) | 6000, 2023-12 → | 10000, 2025-07 → | 473, 2017-08 → |
| **SOL** | 2216 barres, **2020-08-11 →** (6,1 ans) | 6000, 2023-12 → | 10000, 2025-07 → | 317, 2020-08 → |

Les dates de départ correspondent aux dates de listing des paires sur Binance.
Les timeframes courtes sont volontairement plafonnées : 5 ans de bougies 15m
représenteraient ~175 000 lignes par actif pour une valeur analytique faible.

### Dérivés (Binance Futures)

| Métrique | Profondeur | Limite |
|---|---|---|
| **funding.rate** | BTC/ETH **7000 points depuis 2020-04-15** (6,4 ans), SOL 6622 depuis 2020-09 | aucune — pagination `endTime` |
| **oi.value** | **31 jours seulement** | **limite de la source** : Binance ne publie qu'environ 30 jours d'open interest |

La faible profondeur d'OI est enregistrée dans `backfill_state` avec sa raison,
pour qu'aucune étude ne suppose un historique qu'elle n'a pas.

> Note technique : Binance plafonne `fundingRate` à 500 lignes par page malgré
> `limit=1000`. Une première version s'arrêtait donc après une page (5 mois).
> La pagination s'arrête désormais sur absence de progression, pas sur la
> taille de page — d'où le passage de 500 à 7000 points.

### Flux ETF (Farside)

| Actif | Lignes | Période | Fonds |
|---|---|---|---|
| **BTC** | 7267 | **2024-01-11 →** | 12 (ARKB, BITB, BRRR, BTC, BTCO, BTCW, EZBC, FBTC, GBTC, HODL, IBIT, MSBT) |
| **ETH** | 4904 | **2024-07-23 →** | 10 (ETH, ETHA, ETHB, ETHE, ETHV, ETHW, EZET, FETH, QETH, TETH) |

### Macro (Yahoo Finance, sans clé)

10 séries × ~2514 points, **2016-09-06 → 2026-09-04 (10 ans)** :
S&P 500, Nasdaq, Dow, DXY, VIX, WTI, or, rendements 13 semaines / 5 ans / 10 ans.

### Macro (FRED, clé gratuite)

12 séries configurées, non collectées tant que `FRED_API_KEY` est absente :
Fed funds (quotidien et mensuel), 2Y, 10Y, spread 10Y-2Y, CPI, Core CPI, PCE,
Core PCE, chômage, NFP, dollar large.

> `DTWEXBGS` (FRED) est un panier **large pondéré par les échanges** — ce n'est
> pas le DXY coté par les marchés. Les deux sont collectés sous des noms de
> métrique distincts et ne sont jamais confondus.
